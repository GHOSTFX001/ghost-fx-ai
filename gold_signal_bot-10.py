...
def backtest():
    ...
    candles_30m = fetch_candles("30min", 48)
    candles_1m = fetch_candles("1min", 1500)

    if not candles_30m or not candles_1m:
        return "Could not fetch data for backtest.", 500

    # Reference "now" as 24 hours ago -- if run over the weekend, literal
    # "now" lands almost exactly at Friday's market close (since that closing
    # time nearly coincides with the 21:00 UTC session boundary), which
    # would pick up almost no real data. Going back a day lands mid-session
    # instead, correctly capturing the full prior trading day.
    reference_time = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(days=1)
