...
def touches(c, price, tolerance=2.5):
...
def last_weekday_session_start(now_utc):
    """Explicitly finds the 21:00 UTC start of the most recently COMPLETED
    weekday (Mon-Fri) session, skipping weekends entirely. Used for
    backtesting over a weekend, when the live market is closed and clock-
    based guesses keep landing inside the closed/flat period."""
    d = now_utc.date()
    while True:
        d -= datetime.timedelta(days=1)
        candidate_end = datetime.datetime.combine(
            d, datetime.time(SESSION_START_UTC_HOUR), tzinfo=datetime.timezone.utc
        )
        if d.weekday() < 5 and candidate_end <= now_utc:  # Mon-Fri, already closed
            return candidate_end - datetime.timedelta(days=1)

# ---------------------------------------------------------------------------
# A-level detection (30M)
# ---------------------------------------------------------------------------
def detect_a_levels(candles_30m, existing_levels, reference_time=None):
...
def check_confirmation(level, candles_1m):
    ...
    # Look for first touch BEFORE checking any expiry -- using whatever
    # candle data we actually have, regardless of how much wall-clock time
    # has passed since the level formed.
    ...
    # Only now check the 2-day expiry -- after genuinely confirming no touch
    # was found in the available data, not before looking.
    if not level["touched"]:
        if (now - level["formed_at"]) > datetime.timedelta(days=LEVEL_EXPIRY_DAYS):
            level["invalid"] = True
        return None

    # Find where the touch candle sits in the relevant list. The window must
    # include the candle immediately BEFORE the touch too, because the
    # touching candle itself is often the bearish engulfing candle -- its
    # wick dipping into the level IS the touch. Without the prior candle,
    # that valid pair could never be evaluated.
    touch_idx = next((i for i, c in enumerate(relevant) if c["datetime"] == level["touch_time"]), None)
    if touch_idx is None:
        return None  # touch candle has scrolled outside the current fetch

    start_idx = max(0, touch_idx - 1)
    window = relevant[start_idx: touch_idx + CONFIRM_WINDOW_CANDLES + 1]

    for i in range(len(window) - 1):
        c1, c2 = window[i], window[i + 1]
    ...
    # counted from the actual touch forward, not the pre-touch candle we added.
    candles_since_touch = [c for c in relevant if c["datetime"] >= level["touch_time"]]
    if len(candles_since_touch) > CONFIRM_WINDOW_CANDLES + 1:
        level["invalid"] = True
...
@app.route("/backtest")
def backtest():
    """One-off replay of the most recent full session (e.g. Friday, when run
    over the weekend) through the current detection logic. Useful for
    confirming a fix without waiting for the live market to reopen."""
    candles_30m = fetch_candles("30min", 200)
    candles_1m = fetch_candles("1min", 5000)

    print(f"[backtest] fetched 30m candles: {len(candles_30m)} | 1m candles: {len(candles_1m)}", flush=True)
    if candles_30m:
        print(f"[backtest] 30m range: {candles_30m[0]['datetime']} to {candles_30m[-1]['datetime']}", flush=True)
    if candles_1m:
        print(f"[backtest] 1m range: {candles_1m[0]['datetime']} to {candles_1m[-1]['datetime']}", flush=True)

    if not candles_30m or not candles_1m:
        return "Could not fetch data for backtest.", 500

    reference_time = last_weekday_session_start(datetime.datetime.now(datetime.timezone.utc)) + datetime.timedelta(hours=1)

    print(f"[backtest] reference_time chosen: {reference_time} | session_start: {current_session_start(reference_time)}", flush=True)

    levels = detect_a_levels(candles_30m, [], reference_time=reference_time)
    lines = [f"BACKTEST -- {len(levels)} level(s) found for the most recent session:"]

    for lvl in levels:
        signal = check_confirmation(lvl, candles_1m)
        status = "CONFIRMED" if lvl["confirmed"] else ("INVALID" if lvl["invalid"] else "PENDING")
        lines.append(
            f"Level {round(lvl['price'], 2)} formed {lvl['formed_at'].strftime('%m-%d %H:%M UTC')} "
            f"touched={lvl['touched']} -> {status}"
        )
        if signal:
            lines.append(
                f"  SIGNAL: entry={signal['entry']} sl={signal['sl']} tp={signal['tp']} "
                f"@ {signal['time'].strftime('%H:%M UTC')}"
            )

    message = "\n".join(lines)
    send_telegram(message)
    return "<pre>" + message + "</pre>", 200
