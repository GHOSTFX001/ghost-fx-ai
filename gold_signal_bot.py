...
def fetch_candles(interval: str, outputsize: int = 30):
    ...
    try:
        ...
        print(f"[fetch_candles] request failed: {e}")
        return []

    if "values" not in data:
        if data.get("status") == "error":
            print(f"[fetch_candles] TWELVE DATA ERROR: {data.get('message', data)}")
        else:
            print(f"[fetch_candles] unexpected response: {data}")
        return []
...
def check_confirmation(level, candles_1m):
    ...
    if len(window) > CONFIRM_WINDOW_CANDLES + 1:
        level["invalid"] = True  # window closed, no confirmation
        return None
...
def monitoring_loop():
    ...
    while True:
        try:
            candles_30m = fetch_candles("30min", 20)
            candles_1m = fetch_candles("1min", 60)
            ...
            print(f"[heartbeat {now_str}] 30m candles: {len(candles_30m)} | 1m candles: {len(candles_1m)} | "
                  ...
                  f"confirmed: {sum(1 for l in active_levels if l['confirmed'])} | latest 1m: {last_1m_str}")
            ...
            print(f"[monitoring_loop] error: {e}")
