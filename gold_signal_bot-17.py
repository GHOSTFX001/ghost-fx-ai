...
def backtest():
    ...
    # Don't guess the reference time from the clock -- during a weekend
    # closure, Twelve Data pads with flat/repeated candles, and any
    # clock-based guess risks landing on that dead period. Instead, scan
    # backward through the real data to find the last candle that still
    # shows genuine price movement (a real trading moment), and use that.
    reference_time = candles_30m[-1]["datetime"]
    for i in range(len(candles_30m) - 1, 0, -1):
        c, prev = candles_30m[i], candles_30m[i - 1]
        if c["high"] != c["low"] or c["close"] != prev["close"]:
            reference_time = c["datetime"]
            break
